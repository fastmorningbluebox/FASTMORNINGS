...
const SYSTEM = `You are a professional translator specialized in EN, JA, and zh-Hant (Traditional Chinese).
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const completion = await client.chat.completions.create({
      ...
      temperature: 0.2,
      messages: [
        { role: 'system', content: SYSTEM.replace('{TONE}', tone || 'Cute') },
        { role: 'user', content: userPrompt }
      ]
    });

    const translation = completion.choices?.[0]?.message?.content?.trim() || '';
    if (!translation) return res.status(500).send('Empty response');

    res.setHeader('Content-Type', 'application/json');
    res.status(200).send(JSON.stringify({ translation }));
  } catch (err) {
    console.error(err);
